...
const selectCurrentUserTimeRemaining = createSelector(
  [selectRaw],
  (auth) => auth.timeRemaining
);

const selectCurrentTenant = createSelector([selectRaw], (raw) => {
...
// const selectCurrentUserTimeRemaining = createSelector(
//   [currentUserTimeRemaining],
//   (currentUser) => (currentUser ? currentUser.timeRemaining : null)
// );
...
const selectRoles = createSelector(
  ...
  (currentUser, currentTenant) => {
    ...
    const tenantUser = currentUser.tenants.find(
      (userTenant) => userTenant.tenant.id === currentTenant.id
    );
...
const selectLoadingPasswordChange = createSelector([selectRaw], (auth) =>
  Boolean(auth.loadingPasswordChange)
);
...
const selectInvitedTenants = createSelector(
  ...
  (currentUser) => {
    ...
    return currentUser.tenants
      .filter((tenantUser) => tenantUser.status === "invited")
...
const selectBackgroundImageUrl = createSelector(
  ...
  (currentTenant) => {
    ...
    return _get(
      settings,
      "backgroundImageUrl",
      _get(settings, "backgroundImages[0].downloadUrl", null)
    );
  }
);
...
const authSelectors = {
  ...
  selectCurrentUserTimeRemaining,
  selectCurrentUser,
