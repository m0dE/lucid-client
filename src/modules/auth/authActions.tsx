...
import tenantsAction from 'src/modules/tenant/form/tenantFormActions';
...
const authActions = {
  ...
  CURRENT_USER_REFRESH_TIMEREMAINING: `${prefix}_CURRENT_USER_REFRESH_TIMEREMAINING`,
  ...
  doGoogleUpsertAccount: (email) => async (dispatch) => {
    try {
      dispatch({ type: authActions.AUTH_START });

      const token = await service.upsertGoogleAccount(email);
      ...
      if (currentUser && !currentUser.tenants.length) {
        dispatch(
          tenantsAction.doCreate({
            name: urlfy(i18n('tenant.defaultWorkspace')),
          })
        );
      }
  ...
  doRegisterEmailAndPassword: (email, password) => async (dispatch) => {
    try {
      ...
      dispatch(
        tenantsAction.doCreate({ name: urlfy(i18n('tenant.defaultWorkspace')) })
      );
    ...
    async (dispatch) => {
      try {
        ...
        if (doRefresh) {
          ...
          await dispatch({
            type: authActions.CURRENT_USER_REFRESH_TIMEREMAINING,
            payload: parseInt(data.timeRemaining),
          });
        }
      } catch (error) {
        Errors.handle(error);
