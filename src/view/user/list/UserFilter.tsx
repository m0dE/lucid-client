...
const schema = yup.object().shape({
  ...
  role: yupFilterSchemas.enumerator(i18n('user.fields.role')),
...
const previewRenders = {
  ...
  role: {
    ...
    render: (value) => (value ? i18n(`roles.${value}.label`) : null),
  },
  status: {
...
function UserFilter(props) {
  ...
  useEffect(() => {
    // const data = { ...schema.cast(initialValues), admin: true };
  ...
  const onSubmit = (values) => {
    ...
    dispatch(
      actions.doFetch({ ...values, admin: true }, { ...rawValues, admin: true })
    );
    setExpanded(false);
  };
        ...
        <FormProvider {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className='pl-4 pr-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4'>
              <InputFormItem name={'email'} label={i18n('user.fields.email')} />
